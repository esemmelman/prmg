...
export function Editor({ editor, data, onSave, onDelete, onClose, onComment, onDeleteComment }) {
  ...
  const singular = { projects: 'project', tasks: 'task', documents: 'page' }[type]
  const [values, setValues] = useState(() => ({
    ...
    assignee: '', start_date: '', due_date: '', labels: [], checklist: [],
    category: '', pinned: false, completed: false, ...editor.defaults, ...item,
    ...
    <form onSubmit={submit} onChange={() => { dirty.current = true }}>
      <fieldset disabled={busy} className="editor-body">
        ...
        {type === 'documents' && preview ? <article className="markdown document-preview"><div className="eyebrow"><BookOpen size={14}/> {values.category}</div><h1>{values.title || 'Untitled page'}</h1><Markdown>{values.content || '*No content yet. Switch to Write to start your page.*'}</Markdown></article> : <>
          ...
          {type === 'tasks' && <><div className="form-grid"><Field label="Assignee"><input maxLength={160} value={values.assignee} onChange={e => update('assignee', e.target.value)}/></Field><Field label="Labels · comma separated"><input value={values.labels.join(',')} onChange={e => update('labels', e.target.value.split(',').slice(0,20))}/></Field></div>
            ...
            <div className="inline-input"><input aria-label="New subtask" maxLength={240} value={subtask} onChange={e => setSubtask(e.target.value)} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); if(subtask.trim() && values.checklist.length < 100) { update('checklist', [...values.checklist,{ id: crypto.randomUUID(), text: subtask.trim(), done: false }]); setSubtask(''); dirty.current = true } } }}/><button type="button" className="button subtle" disabled={!subtask.trim() || values.checklist.length >= 100} onClick={() => { update('checklist', [...values.checklist,{ id: crypto.randomUUID(), text: subtask.trim(), done: false }]); setSubtask(''); dirty.current = true }}><Plus size={16}/> Add</button></div>
          </>}
        </>}
