...
import { ArrowUpRight, ArrowRight, Plus, MoreHorizontal, CalendarDays, Check, Circle, Flag, BookOpen, Pin, ChevronLeft, ChevronRight, CheckCheck, FolderOpen, ListTodo, Diamond, MessageSquare, LayoutGrid, List, Clock3, Search } from 'lucide-react'
...
export function Timeline({ data, projectId, search, onEdit, onSave }) {
  const [start, setStart] = useState(() => addDays(today(),-3))
  const [days, setDays] = useState(30)
  const end = addDays(start,days - 1)
  const projects = data.projects.filter(p => projectId ? p.id === projectId : p.status !== 'archived')
  const ids = new Set(projects.map(p => p.id))
  const items = [
    ...data.projects.filter(p => ids.has(p.id) && (p.start_date || p.due_date)).map(p => ({...p,type:'projects',title:p.name,project_id:p.id})),
    ...data.tasks.filter(t => ids.has(t.project_id) && (t.start_date || t.due_date)).map(t => ({...t,type:'tasks'})),
    ...data.milestones.filter(m => ids.has(m.project_id)).map(m => ({...m,type:'milestones'})),
  ].filter(i => matches(i,search)).sort((a,b) => (a.start_date || a.due_date).localeCompare(b.start_date || b.due_date))
  const visible = items.filter(i => (i.due_date || i.start_date) >= start && (i.start_date || i.due_date) <= end)
  ...
  return <><div className="view-toolbar"><div className="timeline-nav"><button className="icon-button" aria-label="Previous period" onClick={() => setStart(addDays(start,-days))}><ChevronLeft size={19}/></button><strong>{formatDate(start)} — {formatDate(end,true)}</strong><button className="icon-button" aria-label="Next period" onClick={() => setStart(addDays(start,days))}><ChevronRight size={19}/></button><button className="button secondary small" onClick={() => setStart(addDays(today(),-3))}>Today</button></div><div className="filters"><select aria-label="Timeline period" value={days} onChange={e => setDays(Number(e.target.value))}><option value={14}>2 weeks</option><option value={30}>30 days</option><option value={90}>90 days</option></select><button className="button secondary" disabled={!projects.length} onClick={() => onEdit('milestones')}><Flag size={16}/> New milestone</button></div></div>
    <div className="timeline-scroll"><div className="timeline-chart"><div className="timeline-header"><span>PROJECT / TASK</span><div className="timeline-scale">{Array.from({length: days > 30 ? 6 : days > 14 ? 5 : 7},(_,i) => { const divisions = days > 30 ? 6 : days > 14 ? 5 : 7; const date = addDays(start,Math.floor(days / divisions * i)); return <span key={date}>{dateValue(date).toLocaleDateString(undefined,{month:'short',day:'numeric'})}</span> })}</div></div>
      {visible.length ? visible.map(item => { const first = item.start_date || item.due_date, last = item.due_date || item.start_date; const left = Math.max(0,dayDiff(start,first))/days*100, width = Math.max(1,dayDiff(first < start ? start : first,last > end ? end : last)+1)/days*100; const project = projects.find(p => p.id === item.project_id); return <div className="timeline-row" key={item.id}><button className="timeline-item-name" onClick={() => onEdit(item.type,item)}>{item.type === 'milestones' ? <Diamond size={15}/> : item.type === 'projects' ? <FolderOpen size={15}/> : <Circle size={13}/>}<span>{item.title}<small>{project?.name}</small></span></button><div className="timeline-lane">{today() >= start && today() <= end && <div className="today-line" style={{left:`${dayDiff(start,today())/days*100}%`}}/>}<button title={`${item.title}: ${formatDate(first)} – ${formatDate(last)}`} aria-label={`Edit ${item.title}`} className={`timeline-bar ${item.type === 'milestones' ? 'milestone-bar' : ''} ${item.status === 'done' || item.completed ? 'finished' : ''}`} style={{left:`${left}%`,width:`${Math.min(width,100-left)}%`,'--project-color':project?.color}} onClick={() => onEdit(item.type,item)}>{item.type === 'milestones' ? <Diamond size={18}/> : <span>{item.title}</span>}</button></div></div> }) : <div className="timeline-empty">No scheduled work in this period. Add task dates or browse another period.</div>}
    </div></div><p className="timeline-hint"><span className="legend-line"/> Today <span>Projects and tasks with dates appear here. Click any bar to edit.</span></p>
