...
function TaskCard({ task, data, onEdit, onToggle, compact = false }) {
  return <article className={`task-card ${compact ? 'task-row' : ''}`} draggable={compact ? undefined : true} onDragStart={e => { if(compact) return; e.dataTransfer.setData('text/plain',task.id); e.dataTransfer.effectAllowed = 'move' }}>
    ...
    {task.checklist.length > 0 && <ul className="task-subtasks" aria-label={`Subtasks for ${task.title}`}>{task.checklist.map(item => <li key={item.id}><span className={`complete-button ${item.done ? 'checked' : ''}`} aria-label={item.done ? 'Completed' : 'Incomplete'}>{item.done && <Check size={13}/>}</span><button className={item.done ? 'struck' : ''} onClick={() => onEdit('tasks',task)}>{item.text}</button></li>)}</ul>}
