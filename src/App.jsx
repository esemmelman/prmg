...
import { FolderOpen, LayoutGrid, ListTodo, BookOpen, CalendarDays, Search, Plus, ArrowLeft, Settings2, LogOut, LockKeyhole, ArrowRight, LoaderCircle, RefreshCw, Menu, X, Download, ShieldCheck, CircleHelp, Sprout, Activity, Check } from 'lucide-react'
...
import { Overview, Projects, Tasks, Knowledge, GanttChart, Badge } from './Views'
...
const descriptions = { overview: 'A little perspective for everything you’re working on.', projects: 'A home for every idea, from first step to finish line.', tasks: 'Small steps. Meaningful progress.', knowledge: 'Everything you know, right where you need it.', gantt: 'Plan projects, tasks, and milestones on one schedule.', allProjects: 'Tasks and schedules across all your projects.' }
...
export default function App() {
  ...
  const createType = {overview:'projects',projects:'projects',tasks:'tasks',knowledge:'documents',gantt:'milestones'}[currentView] || 'projects'
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell"><header className="topbar"><div className="topbar-start"><button className="icon-button mobile-menu" aria-label="Open navigation" onClick={() => setSidebar(true)}><Menu size={20}/></button><span className="app-version">v{version}</span></div><div className="topbar-actions"><label className="search-box"><Search size={16}/><input ref={searchRef} aria-label="Search current view" placeholder="Search this view…" value={search} onChange={e => setSearch(e.target.value)}/>{search ? <button className="icon-button" aria-label="Clear search" onClick={() => setSearch('')}><X size={14}/></button> : <kbd>Ctrl K</kbd>}</label><button className="icon-button activity-button" title="Recent activity" aria-label="Recent activity" onClick={() => setShowActivity(true)}><Activity size={19}/></button><span className="topbar-avatar">E</span></div></header>
      ...
      {currentView === 'overview' && <Overview {...props}/>}{currentView === 'projects' && <Projects {...props}/>}{currentView === 'tasks' && <Tasks key={`${projectId || view}-${taskFilter}`} {...props} initialFilter={taskFilter}/>}{currentView === 'knowledge' && <Knowledge key={projectId || 'all'} {...props}/>}{currentView === 'gantt' && <GanttChart key={projectId || 'all'} {...props}/>}
