...
export default function App() {
  ...
  return <div className={`app-shell ${sidebar ? 'sidebar-visible' : 'sidebar-hidden'}`}>
    ...
    <div className="main-shell"><header className="topbar"><div className="topbar-start"><button className="icon-button sidebar-toggle" aria-label={sidebar ? 'Hide navigation' : 'Open navigation'} title={sidebar ? 'Hide sidebar' : 'Show sidebar'} aria-expanded={sidebar} aria-controls="workspace-sidebar" onClick={() => setSidebar(open => !open)}>{sidebar ? <ChevronLeft size={20}/> : <ChevronRight size={20}/>}</button><span className="app-version">v{version}</span></div><div className="topbar-actions"><label className="search-box"><Search size={16}/><input ref={searchRef} aria-label="Search current view" value={search} onChange={e => setSearch(e.target.value)}/>{search ? <button className="icon-button" aria-label="Clear search" onClick={() => setSearch('')}><X size={14}/></button> : <kbd>Ctrl K</kbd>}</label><button className="icon-button activity-button" title="Recent activity" aria-label="Recent activity" onClick={() => setShowActivity(true)}><Activity size={19}/></button><span className="topbar-avatar">E</span></div></header>
