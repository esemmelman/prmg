...
import { FolderOpen, LayoutGrid, ListTodo, BookOpen, CalendarDays, Search, Plus, ArrowLeft, Settings2, LogOut, LockKeyhole, ArrowRight, LoaderCircle, Menu, X, Download, ShieldCheck, CircleHelp, Sprout, Activity, Check } from 'lucide-react'
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell"><header className="topbar"><div className="topbar-start"><button className="icon-button mobile-menu" aria-label="Open navigation" onClick={() => setSidebar(true)}><Menu size={20}/></button><span className="app-version">v{version}</span></div><div className="topbar-actions"><label className="search-box"><Search size={16}/><input ref={searchRef} aria-label="Search current view" placeholder="Search this view…" value={search} onChange={e => setSearch(e.target.value)}/>{search ? <button className="icon-button" aria-label="Clear search" onClick={() => setSearch('')}><X size={14}/></button> : <kbd>Ctrl K</kbd>}</label><button className="icon-button activity-button" title="Recent activity" aria-label="Recent activity" onClick={() => setShowActivity(true)}><Activity size={19}/></button><span className="topbar-avatar">E</span></div></header>
      <main className="main-content" aria-busy={loading}><div className="page-heading"><div>{project && <button className="back-link" onClick={() => { navigate('allProjects'); setProjectTab('tasks') }}><ArrowLeft size={14}/> All Projects</button>}<div className="page-title"><h1>{title}</h1>{project && <Badge value={project.status} labels={PROJECT_STATUSES}/>}</div>{(project || descriptions[view]) && <p>{project ? project.description || '' : descriptions[view]}</p>}</div><div className="heading-actions">{project && <button className="button secondary" onClick={() => edit('projects',project)}><Settings2 size={16}/> Edit project</button>}{createType && createType !== 'projects' && <button className="button primary" onClick={() => edit(createType)}><Plus size={17}/>{createLabel}</button>}</div></div>
      ...
      <footer className="workspace-footer"><span><i className={online && !error ? 'online' : 'offline'}/>{!online ? 'Offline' : error ? 'Sync needs attention' : 'Saved to Supabase'}{lastSync && !error && ` · ${lastSync.toLocaleTimeString([], {hour:'numeric',minute:'2-digit'})}`}</span></footer>
      </main></div>
